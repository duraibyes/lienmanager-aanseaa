    ...
    <div className="">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-8 space-y-6">
          <div>
            <label className="block md:text-sm font-semibold text-slate-700 mb-2">
            ...
            <input
              ...
              className="w-full px-4 py-1.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary/40 focus:outline-none focus:border-primary"
            />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-11 pr-4 py-1.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary/40 focus:outline-none focus:border-primary"
              >
          ...
          <div>
            ...
            <div className="space-y-2">
              {projectTypes.map((option) => (
                <label key={option.id} className={`flex items-start gap-3 p-4 border-2  rounded-lg ${data.projectTypeId === option.id ? "border-primary" : "border-slate-200"} hover:bg-primary/10 cursor-pointer transition-all`}>
                  <input
                    type="radio"
                    name="projectType"
                    value={option.id}
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-8 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {roles.map((option) => (
                <label key={option.id} className={`flex items-start gap-3 p-4 border-2 ${data.roleId === option.id ? "border-primary hover:bg-primary/10" : "border-slate-200"} rounded-lg  cursor-pointer transition-all`}>
                  <input
                    type="radio"
                    name="role"
              ...
              {isCustomerLoading ? (
                ...
                <div className="space-y-2">
                  ...
                  {customerTypesRes?.data?.map((option) => (
                    <label key={option.customer.id} className={`flex items-start gap-3 p-4 border-2 ${data.customerTypeId === option.customer.id ? "border-primary hover:bg-primary/10" : "border-slate-200"} rounded-lg  cursor-pointer transition-all`}>
                      <input
                        type="radio"
                        name="customerType"
